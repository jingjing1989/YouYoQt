...
TestWnd::TestWnd(QWidget *parent)
    : QWidget(parent), query_(nullptr), mainProcessCpuCounter_(nullptr), mainProcessMemCounter_(nullptr), renderProcessCpuCounter_(nullptr),
...
void TestWnd::setupUi() {
  ...
  pushButtonNewBrowser_ = newPushButton("pushButtonNewBrowser", "New Browser with Options");
  ...
  pushButtonQuickSettingForIrregularWnd_ = newPushButton("", "Irregular Window");
  ...
  QLabel *labelMainProcDashboard = new QLabel(QString("Main Process").split(" ", Qt::SkipEmptyParts).join("\n"));
  ...
  QLabel *labelRenderProcDashboard = new QLabel(QString("Render Process").split(" ", Qt::SkipEmptyParts).join("\n"));
...
void TestWnd::onPushButtonNewBrowserClicked() {
  ...
  pCefWnd->setAllowExecuteUnknownProtocolViaOS(checkboxAllowExecuteUnknownProtocolViaOS_->isChecked());
