...
QFont loginDialog::SetSourceHanSans() {
  ...
  if (QFontDatabase::addApplicationFont(":/iconfont/res/iconFont/SourceHanSansCN-Bold.ttf") < 0)
    qWarning() << "SourceHan cannot be loaded !";

  int fontId = QFontDatabase::addApplicationFont(":/iconfont/res/iconFont/SourceHanSansCN-Bold.ttf");
  QStringList fontFamilies = QFontDatabase::applicationFontFamilies(fontId);
  qDebug() << "fontFamilies.size() " << fontFamilies.size();
  qDebug() << "fontFamilies: " << fontFamilies.at(0);
